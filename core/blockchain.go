...
import (
	...
	"github.com/Toinounet21/coreth-mod/core/state/snapshot"
...
func (bc *BlockChain) reprocessState(current *types.Block, reexec uint64) error {
	...
	for current.NumberU64() < origin {
		...
		if time.Since(logged) > 8*time.Second {
			log.Info("Regenerating histo state", "block", current.NumberU64()+1, "target", origin, "remaining", origin-current.NumberU64(), "elapsed", time.Since(start))
			logged = time.Now()
		}
	...
	log.Info("Histo state regenerated", "block", current.NumberU64(), "elapsed", time.Since(start), "nodes", nodes, "preimages", imgs)
	if previousRoot != (common.Hash{}) {
